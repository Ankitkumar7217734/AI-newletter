...
export async function POST(request: NextRequest) {
  ...
  // Require auth to avoid foreign key issues and align with reference flow
  if (!user) {
    return NextResponse.json(
  ...
  try {
    ...
    const { categories, frequency } = body;
    ...
    // Try to send event to Inngest to schedule the newsletter, but don't fail if Inngest isn't configured
    try {
      ...
      console.warn(
        "Inngest scheduling failed; preferences saved without scheduling:",
        schedErr
      );
      return NextResponse.json({
        success: true,
        message:
          "Preferences saved. Scheduling will activate once Inngest is configured.",
        scheduled: false,
      });
    }
  } catch (error) {
